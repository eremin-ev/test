...
5. [Base Installation](#5-base-installation)<br>
   5.1. [Exclude undesired locales, etc.](#51-exclude-undesired-locales-etc)<br>
   5.2. [Install Essential Packages with pacstrap](#52-install-essential-packages-with-pacstrap)<br>
   5.3. [Generate fstab](#53-generate-fstab)<br>
6. [Chroot and Configuration](#6-chroot-and-configuration)<br>
   ...
   6.2. [Set the Time Zone](#62-set-the-time-zone)<br>
   ...
   6.9. [Enable fstrim.timer](#enable-fstrim-timer)<br>
...
(`yes` will be replaced by `prohibit-password` when a user public key is
...
Alternatively, just use `bash` (as the rest of this guide does).
...
Remove a `.d` drop-in configuration file that allows PasswordAuthentication on the target:
...
**If an existing GPT is reused**, remove all undesired partitions:
...
The ESP must be formatted as a FAT12/16/32.  FAT32 is preferable for
internal disks.  The minimum size of a FAT32 filesystem is calculated as [1, 2]:

  - Data Region: 65,525 sectors
  - Reserved: 2 sectors
  - File Allocation Table (FAT): 4 bytes per 65,525 entries ≈ 512 sectors
    (assuming 1 FAT table, using the minimum possible data cluster size of 1 sector)

Total: 65,525 + 2 + 512 = 66,039 sectors

This gives the following figures depending on the disk logical
sector size:

- 4096 * (65,525 + 2 + 512) ≈ 258 MiB
- 512 * (65,525 + 2 + 512) ≈ 33 MiB
...
Note that reported physical size most likely is an emulated one.
Modern disks often use a 4096-byte sector size internally.
Also, a typical internal flash erase block size is 1 MiB.  To
align ESP (and the main root filesystem partition which is next to
it), you need to use multiple of 4096, and even better 1 MiB.

If the reported `LOG-SEC` is 512 bytes, a 64 MiB ESP is acceptable.

Note: Looking ahead, this size is confirmed on Minisforum's MS S1 Max:
the NVRAM firmware does load GRUB from a 64 MiB FAT32 EFI System Partition.
GRUB EFI boot files take a tiny fraction of that space:

```bash
[root@ms-s1-max ~]# du --human-readable /efi/
157K    /efi/EFI/GRUB
157K    /efi/EFI/BOOT
314K    /efi/EFI
314K    /efi/

[root@ms-s1-max ~]# df --human-readable /efi/
Filesystem      Size  Used Avail Use% Mounted on
/dev/nvme0n1p1   63M  314K   63M   1% /efi
```
...
- If `LOG-SEC=512`:
  - Press `Enter` to accept the default first sector: 2048 (= 1 MiB)
  - Type +64M for the last sector
- If `LOG-SEC=4096`:
  - Press `Enter` to accept the default first sector: 256 (= 1 MiB)
...
Re-read the altered partition table manually if necessary (but fdisk usually does 
this automatically):
...
```bash
[root@archiso ~]# efibootmgr --bootnum 0000 --delete-bootnum
```
...
### 5.1 Exclude undesired locales, etc.

Prepare `/etc/pacman.d/noextract.conf` on the live environment:
...
uses it during the initial enrollment:
...
### 5.2 Install Essential Packages with pacstrap
...
[root@archiso ~]# pacstrap -K /mnt base base-devel linux linux-firmware \
    amd-ucode vim grub efibootmgr openssh e2fsprogs dosfstools util-linux less
```
...
By default `pacstrap` creates a stub fstab file containing a useful header:
...
so it is worth appending `genfstab` output rather than overwriting the file completely.
...
Note: you need to remount read-write next time `grub-install` is used.

Make sure it contains the `noatime` mount option for all solid-state drives (SSD,
...
An SSD cannot write data in single-byte increments.  The minimum write unit to
...
Note the prompt has changed to `[root@archiso /]#` in the `chroot` environment.

Fix the `/etc/pacman.conf` in the chroot environment to use `NoExtract` rules from the
...
Replace `Europe/Moscow` with your time zone (e.g., `America/New_York`). Then 
...
- `--removable` – creates a copy of `/EFI/GRUB/grubx64.efi` file to 
  `/EFI/BOOT/BOOTX64.EFI` to protect against motherboard resets: if a CMOS 
  battery dies or the BIOS gets updated, the motherboard will still instantly 
...
For AMD Strix Halo (Ryzen AI Max) systems, the following kernel parameters are 
used to unlock the full potential of the integrated GPU’s unified memory, which 
is crucial for running large language models (LLMs) and other memory-intensive 
GPU workloads:
...
- `amd_iommu=off`: This parameter completely disables the AMD IOMMU 
  (Input-Output Memory Management Unit) driver. The IOMMU normally provides DMA 
  translation and memory protection for devices, but it also adds a small 
  latency overhead to memory accesses. By turning it off, the GPU can access 
  system memory more directly.

  On Strix Halo, the iGPU shares the same physical memory as the CPU. When the 
  GPU frequently reads and writes to system RAM (as it does during LLM 
  inference), the IOMMU translation layer can reduce memory bandwidth. 
  Community benchmarks and guides report that setting `amd_iommu=off` yields a 
  **~6% improvement in memory bandwidth** compared to leaving IOMMU enabled. 
  For workloads that are bandwidth-sensitive, such as large model inference, 
  this gain is significant. The parameter is also recommended in several Strix 
  Halo tuning guides to avoid potential PCI initialization issues.

  > **Note:** If you need RDMA clustering or device passthrough, `iommu=pt` 
  > (pass-through mode) is a safer alternative, but it does not provide the 
  > same bandwidth benefit.

- `amdgpu.gttsize=126976` (in MiBs): restricts the size of the Graphics 
  Translation Table (GTT) domain—the portion of system RAM that the GPU can 
  address as unified memory. The value specifies the maximum amount of system 
  memory the amdgpu driver will make available to the GPU.

  The value is in MiB. The user’s value `126976` therefore corresponds to:

  ```math
  126976 \text{ MiB} \times 1024 = 130023424 \approx 124 \text{ GiB}
  ```

  This means the GPU can access up to ~124 GiB of system RAM.

  Strix Halo’s dedicated VRAM is typically very small (often 512 MB to a few 
  GB), which is insufficient for modern LLMs. The GTT mechanism allows the iGPU 
  to use system RAM as an extension of VRAM. By default, the driver may limit 
  GTT to a fraction of RAM (often 50% or less). Setting `gttsize` to a large 
  value (e.g., 124 GiB) ensures the GPU can allocate large buffers for model 
  weights and activations.

  > **Important:** This parameter is **deprecated** in recent kernels. The 
  > kernel log will warn: *“Configuring gttsize via module parameter is 
  > deprecated, please use ttm.pages_limit”*. However, many Strix Halo guides 
  > still include it for compatibility with older kernels or because it works 
  > alongside `ttm.pages_limit`.

- `ttm.pages_limit=32505856` (in 4 KiB pages): sets the maximum number of 
  memory pages that the TTM (Translation Table Maps) memory manager can pin 
  (lock) in system memory for GPU access. It acts as a hard ceiling on how much 
  RAM the GPU driver can reserve for unified memory.

  On x86-64 Linux, the standard page size is **4 KiB**. Therefore:

  ```math
  32505856 \text{ pages} \times 4 \text{ KiB / page} = 130023424 \text{ KiB} \approx 124 \text{ GiB}
  ```

  This matches the `gttsize` value, ensuring the TTM limit does not become a 
  bottleneck.

  Even if `gttsize` is set high, the TTM subsystem may still restrict 
  allocations to a default fraction of RAM (often half). `ttm.pages_limit` 
  overrides that default, allowing the GPU to pin enough memory for large 
  models. Without it, you may encounter out-of-memory errors when loading 
  models that exceed the default TTM limit.

  > **Note:** In newer kernels, `ttm.pages_limit` is the preferred method for 
  > controlling unified memory size, replacing the deprecated `gttsize` 
  > parameter.
...
run during installation of the kernel package with pacstrap:
...
This guide disables systemd-resolved and writes a static resolv.conf. This
means DNS servers from DHCP are not used.  If you want DHCP DNS, keep
...
#### 6.8.6 Enable systemd-networkd and sshd

Enable the services to ensure they start automatically at boot:
...
### 6.9 Enable fstrim.timer

Running `fstrim` on Linux tells an SSD which data blocks are no
longer in use so the hardware can erase them ahead of time.

Why does `fstrim` matter?  It sustains write speed: when a file gets deleted,
the operating system marks the space as free, but the SSD controller still
thinks the blocks contain data. Without trimming, the drive must perform a
slow read-erase-write cycle later when new data arrives. Trimming lets the
drive clear those blocks during idle periods.
...
Copy live environment’s root SSH `authorized_keys` into the new system

```
[root@archiso ~]# install -d -m 700 /mnt/root/.ssh
[root@archiso ~]# cp -v ~/.ssh/authorized_keys /mnt/root/.ssh/
[root@archiso ~]# chmod 600 /mnt/root/.ssh/authorized_keys
```
...
```bash
[root@ms-s1-max ~]# networkctl status
[root@ms-s1-max ~]# ip addr show
```
...
```bash
[root@ms-s1-max ~]# ping -c 3 ping.archlinux.org
[root@ms-s1-max ~]# getent hosts ping.archlinux.org
```

Ensure the SSH server is enabled and running:

```bash
[root@ms-s1-max ~]# systemctl is-enabled sshd
enabled
[root@ms-s1-max ~]# systemctl is-active sshd
active
```

Create a non-root user for daily use, fixing the primary group ID if necessary:

```bash
[root@ms-s1-max ~]# groupmod --gid 100 <group>
[root@ms-s1-max ~]# useradd --create-home --uid <uid> --gid <group> --shell /bin/bash <username>
[root@ms-s1-max ~]# passwd <username>
```

Changing the group and user ID may be useful if there is an existing setup with
specific user and group IDs.  Logging in to a new machine from the old one via
SSH/rsync or mounting an NFS directory will be much more convenient if UIDs and
GIDs on both machines match.

Add a new drop-in configuration file to `/etc/sudoers.d/` to allow the user
to issue administrative commands (use `visudo -f /etc/sudoers.d/users` if you
need to validate the syntax):

```bash
[root@ms-s1-max ~]# cat > /etc/sudoers.d/users <<EOF
##
...
[root@ms-s1-max ~]# chmod 440 /etc/sudoers.d/users
```
...
Both forms allow the user `<username>` to run a command on behalf of any existing
user in the system using `sudo -u <any-user> <command>`.

The `(ALL:ALL)` syntax stands for `(User_List : Group_List)` so the second
form allows the user to run a process on behalf of any existing group with
`sudo -g <any-group> <command>`.

Copy SSH authorized keys to allow logging in as a regular user instead of root:

```bash
[root@ms-s1-max ~]# cp -v ~root/.ssh/authorized_keys ~<username>/.ssh/
[root@ms-s1-max ~]# chown -Rv <username>:<group> ~<username>/.ssh/
[root@ms-s1-max ~]# chmod -Rv go-rwx ~<username>/.ssh/
```

Set the "screensaver" to blank the monitor after one minute of inactivity:
```bash
[root@ms-s1-max ~]# setterm --blank 1 </dev/tty1
```

Please note that a `setterm --blank force </dev/tty1` command not only
blanks the monitor immediately but also disables any keyboard input.
As a result, the monitor will not turn on when a keyboard key is pressed (as one would
expect).  Use the `poke` mode to recover from this unexpected `setterm` behavior:
